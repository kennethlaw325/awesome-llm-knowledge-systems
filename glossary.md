...
An interpretability tool trained on a model's internal activations to decompose them into a much larger set of sparsely active, individually meaningful features --- the working instrument of **Mechanistic Interpretability**. Once trained, the features are usable as development tooling rather than only as analysis: inference-time steering that manipulates internal features without prompt engineering, evaluation analysis that proxies benchmark redundancy and capability coverage, data classification and organization, and post-training support for supervised fine-tuning and RL. Alibaba's **Qwen-Scope** (arXiv 2605.11887, submitted May 12, 2026) is the open instance built for a frontier open-weight family --- 14 SAE groups across seven Qwen3 and Qwen3.5 variants, dense and MoE, spanning 1.7B to 35B parameters (Qwen3-1.7B through Qwen3.5-35B-A3B). See Chapter 9.

**State Supersession**
The case where a fact, a constraint, or a decision held in **Agent Memory** is revised partway through a long interaction, so the stored value no longer holds. It names a failure that is structurally invisible to the recall-oriented benchmarks the field standardized on: a store that confidently returns a value the user *used* to hold scores as a hit. **StateMemBench** (arXiv 2608.19652, August 20, 2026) made it a third memory-governance evaluation axis next to access control and sycophancy, with 234 multi-session scenarios and closed-pool grading that isolates state-tracking failures from ordinary retrieval error; the same paper's StateMem method tracks supersession and relational dependencies explicitly rather than leaving them to retrieval. See Chapter 6.
...
**Temporal Policies**
A gateway-enforced authorization primitive shipped by AWS in Amazon Bedrock AgentCore on August 6, 2026. Where an ordinary policy asks "may this agent make this call," a temporal policy evaluates the call against the agent's *prior* actions in the same session --- deterministically, deny-by-default, fully logged, and outside agent code, so no prompt or model decision can talk its way past it. Policies are written in **Dogwood**, a new Apache-2.0 open-source policy language purpose-built for AI agents. The named uses are the failure modes instructions cannot prevent: data-fabrication prevention (a value the agent passes onward must match what an earlier call actually returned), cumulative session cost caps, and workflow sequencing that requires a human approval step before a privileged action. Generalizes the single-action hard cap (AgentCore Payments, May 2026) from one call to a session history --- the Authority axis of harness design gaining a stateful enforcement layer.
