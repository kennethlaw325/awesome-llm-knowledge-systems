...
### 2026 Caution: A Contested 12M-Token Claim

Not every long-context announcement survives scrutiny. The startup **Subquadratic** launched **SubQ** on May 5, 2026, claiming a **12-million-token window** via **Subquadratic Selective Attention (SSA)**; the company-cited and Appen-run benchmarks are striking --- 56x faster than FlashAttention, 98% needle-in-a-haystack recall at 6-12M tokens, and $8 to run RULER-128 against $2,600 for Claude Opus 4.6. A June 19, 2026 MIT Technology Review examination gave the claim a colder read. Researcher pushback (Will Depue among them) noted that the public evidence does not yet support the stronger "solved quadratic attention" framing, and that SubQ **reuses Qwen weights rather than training from scratch** --- which makes the efficiency numbers a property of a specific adapted model, not a demonstrated general result. The repo's no-hype rule applies here: SSA is a genuinely interesting claim and an unverified one at the same time, and long-context buyers should treat the 12M figure as contested until independent replication lands.
...
### 2026: RAG Becomes a Managed Cloud Primitive

The Context Engine reframe had a managed-infrastructure echo in mid-2026. On **June 17, 2026**, AWS shipped **Amazon Bedrock Managed Knowledge Base** at GA: a fully-managed RAG service with six built-in connectors (S3, SharePoint, Confluence, Google Drive, OneDrive, Web Crawler), automatic sync, managed vector storage, and hybrid search --- plus **"agentic retrieval"** that auto-orchestrates query planning, interim-response evaluation, and re-ranking for multi-hop questions. The beat is not the feature list but the shift it marks: the assembly pipeline this chapter calls a Context Engine is moving from a component teams hand-build toward a **commodity cloud primitive** teams provision. As with the managed-agent substrate (Chapter 4), the DIY version does not disappear --- but the default for "stand up RAG over our documents" increasingly becomes a configuration choice rather than an engineering project.
...
- Goyal, Ankit and Ray, Jaideep. "Does Your Agent's Memory Survive a Model Upgrade? A Controlled Study of Memory Portability." arXiv 2609.05339 (September 4, 2026). [https://arxiv.org/abs/2609.05339](https://arxiv.org/abs/2609.05339) --- 48 synthetic cases across raw history, RAG, compressed notes, and structured knowledge graphs; graph memory near-invariant to model swaps (+0.0004 ± 0.0020) while compressed notes swing +9.91 / -13.28 points. Small synthetic sample, two authors, unreplicated. Covered in full in Chapter 6.
