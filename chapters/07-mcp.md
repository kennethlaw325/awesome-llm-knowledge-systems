...
**Beta SDKs and the finalization window (June 29).** On June 29, 2026, the MCP project shipped beta SDKs across Python, TypeScript, Go, and C#, opening a four-week testing period before the July 28, 2026 specification finalization. The TypeScript v2 SDK splits the previously monolithic package into separate `@modelcontextprotocol/server` and `@modelcontextprotocol/client` packages. The betas also carry **Multi Round-Trip Requests (SEP-2322, Status: Final, created 2026-02-03)**, which re-plumbs how server-initiated requests --- including sampling and elicitation --- are delivered: it replaces the held-open SSE stream and server-side state with a *stateless retry*, where the tool returns an `InputRequiredResult` and the client re-issues the call. What sampling and elicitation can do is unchanged; SEP-2322 changes only how they are transported, so the bidirectional capabilities now fit the stateless core rather than requiring a long-lived connection.
...
The IETF draft named authentication-enforcement gaps and enterprise SSO friction as open problems; the same month, the MCP maintainers shipped a stable answer to the enterprise half. **Enterprise-Managed Authorization (EMA)** --- announced by core maintainer **Paul Carleton** on June 18, 2026 --- lets an organization provision MCP server access centrally through its identity provider. A user signs in once and finds their sanctioned servers already connected, with no per-app OAuth consent screen for each one.

The mechanism is a two-step token exchange. During SSO, the client obtains an **Identity Assertion JWT Authorization Grant (ID-JAG)** from the IdP; it then exchanges that grant for an access token issued by the MCP server's *own* authorization server. Neither the user nor the client ever runs the classic per-server OAuth consent dance --- the IdP vouches for the user, and each MCP server's authorization server decides what that vouching buys.

The notable part is the day-one breadth, which is what separates a real standard from a proposal: **Okta** as an IdP (via its Cross App Access extension), **Anthropic** (Claude, Claude Code, Cowork) and **VS Code** as clients, and seven servers --- Asana, Atlassian, Canva, Figma, Granola, Linear, Supabase --- with Slack actively adding support. For the knowledge harness, EMA closes the *"SSO-integrated authentication remains friction-heavy"* gap this chapter's own 2026 Challenges section names: enterprise SSO stops being per-server glue and becomes a provisioning decision made once at the IdP.

Provisioning-time authorization is not per-call authorization, though, and on **September 10, 2026** Anthropic answered the second half with an `auto` permission policy for Managed Agents: the server, not the client, evaluates each individual tool call and then runs it, denies it with an error the client cannot override, or pauses it for approval --- a policy that applies, per Anthropic's documentation, to MCP toolsets as well as the built-in agent toolset, putting individual MCP tool calls under platform-level governance and sitting alongside AWS AgentCore's deterministic Temporal Policies in [Chapter 4](04-harness-engineering.md).

## 2026-07-28: The Specification Ships Final

The ten-week validation window closed on schedule. On **July 28, 2026**, the MCP maintainers (David Soria Parra and Den Delimarsky) shipped the finalized **2026-07-28 specification** --- the largest revision since authorization was added, and confirmation that the release-candidate arc this chapter has tracked since May 21 landed as designed. The core is now **stateless request / response**: the `initialize` / `initialized` handshake and `Mcp-Session-Id` headers are gone for good, **Multi Round-Trip Requests** (SEP-2322) carries server-initiated interactions over stateless retries, and the **extensions framework** is formal --- Tasks now lives at `io.modelcontextprotocol/tasks` rather than in the protocol core.

Authorization hardened one step beyond the RC. The final spec mandates **RFC 9207 issuer validation** and replaces Dynamic Client Registration with **Client ID Metadata Documents (CIMD)** --- clients are identified by a hosted metadata document rather than a per-server registration dance. Roots, Sampling, Logging, and the original HTTP+SSE transport are deprecated with a minimum 12-month removal window. All four Tier 1 SDKs (Python, TypeScript, Go, C#) shipped matching updates, with endorsements from AWS, Google Cloud, Microsoft, and Cloudflare.
...
17. Model Context Protocol blog. "Enterprise-Managed Authorization" (June 18, 2026). [https://blog.modelcontextprotocol.io/posts/enterprise-managed-auth/](https://blog.modelcontextprotocol.io/posts/enterprise-managed-auth/) and specification [https://modelcontextprotocol.io/extensions/auth/enterprise-managed-authorization](https://modelcontextprotocol.io/extensions/auth/enterprise-managed-authorization) --- announced by core maintainer Paul Carleton; ID-JAG token exchange; day-one Okta (IdP) / Anthropic + VS Code (clients) / seven servers.
18. Model Context Protocol blog. "MCP SDK betas ahead of the 2026-07-28 finalization" (June 29, 2026). [https://blog.modelcontextprotocol.io/posts/sdk-betas-2026-07-28/](https://blog.modelcontextprotocol.io/posts/sdk-betas-2026-07-28/) and SEP-2322 [https://modelcontextprotocol.io/seps/2322-MRTR](https://modelcontextprotocol.io/seps/2322-MRTR) --- Python / TypeScript / Go / C# betas; four-week testing period before July 28 finalization; TypeScript v2 server/client package split; Multi Round-Trip Requests (SEP-2322, Final, created 2026-02-03).
19. Model Context Protocol blog. "The 2026-07-28 specification" (July 28, 2026). [https://blog.modelcontextprotocol.io/posts/2026-07-28/](https://blog.modelcontextprotocol.io/posts/2026-07-28/) --- final spec: stateless core, MRTR (SEP-2322), RFC 9207 issuer validation, CIMD replacing Dynamic Client Registration, extensions framework (`io.modelcontextprotocol/tasks`), 12-month deprecation window for Roots / Sampling / Logging / HTTP+SSE.
...
24. Anthropic. Permission policies, Managed Agents documentation (`auto` policy dated September 10, 2026 via the platform release notes). [https://platform.claude.com/docs/en/managed-agents/permission-policies](https://platform.claude.com/docs/en/managed-agents/permission-policies) and release notes [https://platform.claude.com/docs/en/release-notes/overview](https://platform.claude.com/docs/en/release-notes/overview) --- server-side per-call evaluation returning run, deny, or pause-for-approval, reported through an `evaluation` object on `agent.tool_use` and `agent.mcp_tool_use` events with reason codes such as `high_risk` and `indeterminate`; applies to MCP toolsets, whose policy defaults to `always_ask` unless explicitly overridden. Single-sourced to Anthropic's documentation; no independent coverage located.
