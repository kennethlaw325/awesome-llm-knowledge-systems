...
Mid-2026's governance benchmarks (above) all still assume memory is *read through an interface* --- a vector index, a graph, a curator pass. On July 29, 2026, **MemTensor** (Shanghai) --- the same lab behind the June 23 agent-native-memory survey above, this time joined by co-authors from Renmin University, NUS, SJTU, and Tongji (17 authors total) --- pushed the taxonomy in a different direction. **Metis** ("Metis: Training Large Language Models as Memory Foundation Models," arXiv 2607.26760) proposes **memory foundation models**: agent memory as persistent, dynamically-evolving state that lives *inside the transformer backbone itself*, parametric rather than external. Where the four-module survey above assumes an agent reads memory through Representation & Storage / Extraction / Retrieval & Routing / Maintenance modules sitting outside the model, Metis's prototype --- shipped at 4B, 9B, and 27B scale on Qwen3.5 backbones --- freezes the backbone and trains only a **hyper memory block** and **local memory block**, explicitly inspired by Fast Weight Programming (FWP).
...
22. "Metis: Training Large Language Models as Memory Foundation Models." arXiv 2607.26760 (submitted July 29, 2026; MemTensor / Renmin University / NUS / SJTU / Tongji, 17 authors). https://arxiv.org/abs/2607.26760 --- parametric native memory inside a frozen transformer backbone (4B/9B/27B on Qwen3.5); hyper memory block + local memory block, FWP-inspired; gradient-free EMA-style inference-time updates; self-reported limitations: long-horizon information loss under fixed-size compression, latent-space semantic blending.
