...
The Model Context Protocol Steering Committee locked the **2026-07-28 release candidate**, opening a ten-week validation window before the final spec ships July 28, 2026. Five primitives change the interop surface at once. (1) **Stateless protocol core**: the `initialize` / `initialized` handshake and `Mcp-Session-Id` headers are removed; client metadata travels in `_meta` on every request, so any MCP request can land on any server instance. This directly unblocks horizontal scaling behind load balancers --- the largest production gap from the 2026 roadmap. (2) **Extensions framework**: capabilities identified by reverse-DNS IDs and negotiated through an `extensions` map, with a formal Standards Track pathway --- the spec gains a way to version independently of its core. (3) **MCP Apps**: servers can ship interactive HTML interfaces that hosts render in sandboxed iframes, with UI templates declared upfront for security review and caching --- the first MCP-native deliverable that is not a tool call. (4) **Tasks** (SEP-1686) moves from experimental core to extension status with a redesigned stateless lifecycle (`tools/call` returns task handles; clients drive progress via `tasks/get` / `tasks/update` / `tasks/cancel`). (5) **Authorization hardening** (six SEPs) tightens OAuth 2.0 / OIDC alignment with mandatory `iss` parameter validation and application-type declarations. A formal **deprecation policy** also lands: Roots, Sampling, and Logging transition to deprecated with a minimum 12-month removal window. After 18 months of feature growth, the spec spends a release cycle hardening the foundations under it.
